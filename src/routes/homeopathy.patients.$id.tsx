...
import { ArrowLeft, Sparkles, Phone, MapPin, Calendar } from "lucide-react";
  ...
  if (!p) {
    return (
      <Card>
        ...
        <Link to="/homeopathy/queue" className="text-sm text-primary hover:underline">
        ...
        {historyQ.isLoading ? (
          ...
          <ol className="relative pl-5 space-y-4">
            ...
            {visits.map((v, i) => {
              ...
              const remedy = v.remedy || v.homeopathy?.remedy || (Array.isArray(v.medicines) && v.medicines[0]?.name) || "—";
              const potency = v.potency || v.homeopathy?.potency;
              return (
                <li key={v.id ?? i} className="relative">
                  ...
                  <div className="text-xs text-primary inline-flex items-center gap-1 mt-1">
                    <Sparkles className="size-3" /> {remedy}{potency ? ` · ${potency}` : ""}
                  </div>
                </li>
              );
            })}
